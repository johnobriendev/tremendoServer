...
describe('Board Controller', () => {
  ...
  describe('PUT /boards/:id', () => {
    ...
    it('should return 403 if board does not exist or user is not owner', async () => {
      ...
      const res = await request(app)
        .put(`/boards/${nonExistentBoardId}`)
        .set('Authorization', `Bearer ${token}`)
        .send({ name: 'Non-existent Board' });

      expect(res.status).toBe(403);
      expect(res.body.message).toBe('Not authorized to access this board');
    });
  });
  ...
  describe('DELETE /boards/:id', () => {
    ...
    it('should return 403 if board does not exist or user is not owner', async () => {
      ...
      const res = await request(app)
        .delete(`/boards/${nonExistentBoardId}`)
        .set('Authorization', `Bearer ${token}`);
...
//       const res = await request(app)
//         .put(`/boards/${nonExistentBoardId}`)
//         .set('Authorization', `Bearer ${token}`) // Using token of authorized user
//         .send({ name: 'Non-existent Board' });
